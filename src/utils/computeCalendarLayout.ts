import type { CalendarData, EventDisplayProps, Week, Event } from "../types/types"

export function computeCalendarLayout(year: number, month: number, events: Event[]): CalendarData {
          ...
          if (event.start <= dayEnd && event.end >= day) {
            ...
            while (!placed) {
              ...
              if (canPlaceEvent) {
                ...
                week.eventDetails[row].push({ ...event, eventDisplayProps })
